			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("ModifiedBy") === oObject.ModifiedBy;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].ModifiedBy;
									} else {
										sCurrentId = aItemsNotInTheList[0].ModifiedBy;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("jbcommon_auth_ModifiedBy", {
										ModifiedBy : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("ModifiedBy"),
						title: oBindingContext.getProperty("ModifiedByFullName")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<jbcommon_auth_ModifiedBy> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("ModifiedByFullName");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all jbcommon_auth_ModifiedBy to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<jbcommon_auth_ModifiedBy> (" + iExpectedLength + ")"}),
								success : function () {
